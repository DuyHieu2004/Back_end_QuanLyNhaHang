	...
	public class StatisticsController : ControllerBase
	{
        ...
        public async Task<IActionResult> GetDashboardStats([FromQuery] string timeRange = "TODAY")
        {
            try
            {
                ...
                // Nếu không có stored procedure, tính toán trực tiếp từ database
                try
                {
                ...
                catch (SqlException sqlEx) when (sqlEx.Number == 2812 || sqlEx.Number == 208) // Stored procedure không tồn tại
                {
                    // Fallback: Tính toán trực tiếp từ database nếu stored procedure không tồn tại
                    var today = DateTime.Today;
                    DateTime startDate = safeTimeRange switch
                    {
                    ...
                    var revenueQuery = _context.DonHangs
                        .Where(dh => dh.MaTrangThaiDonHang == "DA_HOAN_THANH" &&
                                     ...
                                     dh.ThoiGianKetThuc.Value.Date >= startDate)
                        .SelectMany(dh => dh.ChiTietDonHangs)
                        .Select(ct => ct.SoLuong * (ct.MaCongThucNavigation.Gia));

                    result.TongDoanhThu = await revenueQuery.AnyAsync() 
                        ? await revenueQuery.SumAsync() 
                        : 0m;

                    // Đếm số đơn hoàn thành
                    result.SoDonHoanThanh = await _context.DonHangs
                        .CountAsync(dh => dh.MaTrangThaiDonHang == "DA_HOAN_THANH" &&
                                          dh.ThoiGianKetThuc.HasValue &&
                                          dh.ThoiGianKetThuc.Value.Date >= startDate);

                    // Đếm số bàn đang phục vụ
                    result.SoBanPhucVu = await _context.BanAns
                        .CountAsync(b => b.MaTrangThai == "TTBA002"); // Đang phục vụ

                    // Đếm tổng khách hàng
                    result.TongKhachHang = await _context.KhachHangs.CountAsync();
                }
