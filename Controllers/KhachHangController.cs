    ...
    public class KhachHangController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetThongKe()
        {
            try
            {
                ...
                // Tính số lần ăn từ số đơn hàng đã hoàn thành
                var khachHangThanThiet = await _context.KhachHangs
                    .Where(k => _context.DonHangs
                        .Count(d => d.MaKhachHang == k.MaKhachHang && d.MaTrangThaiDonHang == "DA_HOAN_THANH") >= 5)
                    .CountAsync();
        ...
        public async Task<IActionResult> GetKhachHangs([FromQuery] string search = "", [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
        {
            try
            {
                ...
                var khachHangs = await query
                    ...
                    {
                        KhachHang = k,
                        SoLanAnTichLuy = _context.DonHangs
                            .Count(d => d.MaKhachHang == k.MaKhachHang && d.MaTrangThaiDonHang == "DA_HOAN_THANH"),
                        LanCuoiDen = _context.DonHangs
                            .Where(d => d.MaKhachHang == k.MaKhachHang && d.MaTrangThaiDonHang == "DA_HOAN_THANH")
                            .OrderByDescending(d => d.ThoiGianDatHang)
                            .Select(d => (DateTime?)d.ThoiGianDatHang)
                            .FirstOrDefault()
                    })
                    .OrderByDescending(x => x.SoLanAnTichLuy)
                    .Skip((page - 1) * pageSize)
                    .Take(pageSize)
                    .Select(x => new KhachHangListDto
                    {
                        MaKhachHang = x.KhachHang.MaKhachHang,
                        HoTen = x.KhachHang.HoTen,
                        SoDienThoai = x.KhachHang.SoDienThoai,
                        Email = x.KhachHang.Email,
                        HinhAnh = x.KhachHang.HinhAnh,
                        SoLanAnTichLuy = x.SoLanAnTichLuy,
                        NoShowCount = x.KhachHang.NoShowCount,
                        NgayTao = x.KhachHang.NgayTao,
                        TongChiTieu = 0.0m,
                        LanCuoiDen = x.LanCuoiDen
                    })
                    .ToListAsync();
        ...
        public async Task<IActionResult> GetKhachHangDetail(string id)
        {
            try
            {
                ...
                // 2. Tính số lần ăn từ đơn hàng đã hoàn thành
                var soLanAnTichLuy = await _context.DonHangs
                    .Where(d => d.MaKhachHang == id && d.MaTrangThaiDonHang == "DA_HOAN_THANH")
                    .CountAsync();
        ...
        public async Task<IActionResult> ExportKhachHangs([FromQuery] string search = "")
        {
            try
            {
                ...
                var khachHangs = await query
                    ...
                    {
                        KhachHang = k,
                        SoLanAn = _context.DonHangs
                            .Count(d => d.MaKhachHang == k.MaKhachHang && d.MaTrangThaiDonHang == "DA_HOAN_THANH")
                    })
                    .OrderByDescending(x => x.SoLanAn)
                    .Select(x => new
                    {
                        MaKhachHang = x.KhachHang.MaKhachHang,
                        HoTen = x.KhachHang.HoTen,
                        ...
                        Email = x.KhachHang.Email ?? "",
                        SoLanAn = x.SoLanAn,
                        NoShowCount = x.KhachHang.NoShowCount ?? 0,
                        NgayTao = x.KhachHang.NgayTao,
                        TongChiTieu = 0.0m
                    })
                    .ToListAsync();
